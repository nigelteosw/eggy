...
- A tool that belongs to one kind of turn declares `TurnScoped` on its definition, and the full catalog an owner turn runs on leaves it out; only an allowlist that names it offers it. `heartbeat_respond` is the one user. An owner turn must not pay a schema it can only fail on.
...
- `/mode` (`state.ApprovalMode`) is the one setting deciding how much the owner is asked: `strict` gates every tool call, `normal` gates what writes, `auto` gates nothing. `normal` is the default and the rung that has to be right -- gating everything trains the owner to approve without reading, and gating nothing is a bypass. `auto` is a deliberate, durable bypass named by `/status`; it must never become the default, and nothing may select it on the owner's behalf. `approvals.mode` in config decides only where a fresh deployment starts: a stored choice always wins, or `/mode` would be undone by the next restart. The retired `state.ApprovalAutoMode` boolean is read once to carry an existing bypass into `auto` and never written again.
...
- Heartbeats are per account and off until the account switches its own on (`/heartbeat on`, or the panel's personal settings). `heartbeat.interval` sets the deployment's cadence and nothing more; nothing may switch a person's heartbeat on for them.
...
- Owner-facing documents load their built-in default when missing, blank, or
  unreadable, and the default is never written into the owner's file. Writing
  `initialSoul` on first read meant an improved soul never reached an existing
  deployment, an emptied SOUL.md left Eggy with no identity, and an unreadable
  one failed every turn. Reads are lenient; writes stay strict and refuse to
  overwrite a file they cannot read.
