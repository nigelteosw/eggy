...
- Any protected mutation keeps one `approvals.Action`, one executor, and one payload-bound approval per operation; consolidating tools is fine, consolidating their approvals is not. `services.ApprovalToolCall` is the mechanism's one registered action: a tool named under an MCP server's `require_approval` is wrapped by `services.NewApprovalGatedTool` at the provider boundary in bootstrap, and `services.ApprovalToolExecutor` authorizes and runs it. A server without `require_approval` is still trusted wholesale at configuration time, which is the default. Google's tools take the same route through `services.NewApprovalGatedToolIf`, which narrows the gate to the calls a `services.ApprovalRule` names — a tool carrying several operations behind one schema must gate the mutating actions and only those, because an approval prompt in front of reading the calendar teaches the owner to approve without looking. The rule is a filter on one mechanism, never a second one: an ungated call runs inline exactly as it would unwrapped. Which Google actions mutate is declared in `google.Mutations()`, beside the switch statements that implement them, with `google.Reads()` as its complement. `google.Actions()` is derived from the two rather than written a third time, which is what makes the classification enforceable: an action left out of both is missing from `Actions()`, so the bootstrap test comparing it against the tool schemas fails, and whoever added it has to say which kind it is before it can ship. Writing `Actions()` by hand would let a new write land ungated and silent.
