...
- `config.yaml` itself on the Advanced page, with the restart that applies it.

Changes are written to `config.yaml`. Restart `eggyd` to reconstruct adapters and apply them: the **Restart** button on the Advanced page does this without a redeploy, as does `/restart` in chat, and every save says so. A config Eggy could not load is refused there rather than applied — see [Restarting to apply config](/eggy/configure/configuration/#restarting-to-apply-config). Secrets are never returned through the configuration API.

Two of those sections are not config and do not need a restart. The tool list is read live from the registry the agent loop itself runs on, so an MCP server that reconnects or is logged out of changes the page for the same reason it changes the next turn. Cancelling a schedule removes its record and takes effect immediately. Approvals are read there and decided through the same route a chat tap uses, so the panel is a second view onto one mechanism rather than a second way to approve something.
