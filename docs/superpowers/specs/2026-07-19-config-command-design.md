...
5. On success, atomic write: temp file in the same directory as `config.yaml`, mode `0600`, `Write`, `Sync`, `Close`, then `os.Rename` over the original — the identical sequence `initializeConfig` already uses in `config_init.go` — wrapped in the same `filelock.With(path, ...)` used there, so a concurrent `/config set` and daemon boot can't race on the file.
