...
func validateHomeFile(relative, content string) error {
	...
	case relative == "config.yaml":
		// Decoded through the same strict path LoadConfig uses, so a typo in
		// a key name is caught here rather than silently ignored until the
		// setting it was meant to change fails to take effect.
		var document configDocument
		if err := decodeKnownYAML([]byte(content), &document); err != nil {
