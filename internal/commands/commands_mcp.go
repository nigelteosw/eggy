...
const mcpExplanation = "MCP (Model Context Protocol) lets Eggy call tools exposed by an MCP server, either a hosted one such as Railway (transport: streamable-http, given by url) or a local subprocess Eggy spawns itself (transport: stdio, given by command and args). Servers are defined in config.yaml's mcp.servers map, not through /mcp or the CLI — see the mcp.servers blocks in config.example.yaml for complete examples of both, including auth mode, tool filter, and, for stdio, the env_allowlist that decides which environment variables the child may see. After adding a server: an oauth server needs EGGY_ENCRYPTION_KEY set so its credentials can be stored encrypted, a bearer-env server needs its named token environment variable set, then restart Eggy either way. A stdio server needs nothing beyond a command Eggy can execute, and any environment variable it depends on named in its env_allowlist. An oauth server also needs /mcp login <server> once Eggy is back up; a bearer-env or none server is ready to use immediately. Once a server is configured: /mcp lists servers and status, /mcp status <server> and /mcp probe <server> inspect one, /mcp login <server> and /mcp logout <server> manage OAuth, and /mcp reload <server> reconnects it and re-reads its tool catalog. Only adding or editing a server needs a restart; reconnecting, reloading, logging in, and logging out all take effect on the next turn."
