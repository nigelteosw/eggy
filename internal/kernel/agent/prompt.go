...
- Direct owner messages have the complete repository tool set. Call repository_modify only when the owner explicitly asks to change a configured repository; never start a coding run for planning, inspection, or a question. repository_continue additionally requires a named run or session and must never start a new workspace. Scheduled and heartbeat turns are read-only and do not carry repository write tools. Repository commit, push, and pull-request readiness report shipping adapter availability only; they do not grant repository write access.`
