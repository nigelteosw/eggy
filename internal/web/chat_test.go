...
func TestThreadRenameOverwritesAnAutoGeneratedTitle(t *testing.T) {
	...
	if err := memory.SetThreadTitle(context.Background(), "thread-1", "auto title"); err != nil {
		t.Fatal(err)
	}
	handler := newThreadRenameHandler(memory)

	request := withPathValue(httptest.NewRequest(http.MethodPatch, "/api/chat/threads/thread-1", strings.NewReader(`{"title":"  Groceries  "}`)), "id", "thread-1")
	response := httptest.NewRecorder()
	handler(response, request)
	...
	thread, _, err := memory.GetThread(context.Background(), "thread-1")
	if err != nil {
		t.Fatal(err)
	}
...
func TestThreadRenameRejectsABlankTitle(t *testing.T) {
	...
	request := withPathValue(httptest.NewRequest(http.MethodPatch, "/api/chat/threads/thread-1", strings.NewReader(`{"title":"   "}`)), "id", "thread-1")
	response := httptest.NewRecorder()
	handler(response, request)
...
func TestThreadRenameReturns404ForAnUnknownThread(t *testing.T) {
	...
	request := withPathValue(httptest.NewRequest(http.MethodPatch, "/api/chat/threads/missing", strings.NewReader(`{"title":"x"}`)), "id", "missing")
	response := httptest.NewRecorder()
	handler(response, request)
...
func TestThreadDeleteRemovesTheThreadAndItsMessages(t *testing.T) {
	...
	request := withPathValue(httptest.NewRequest(http.MethodDelete, "/api/chat/threads/thread-1", nil), "id", "thread-1")
	response := httptest.NewRecorder()
	handler(response, request)
	...
	if _, found, err := memory.GetThread(context.Background(), "thread-1"); err != nil || found {
		t.Fatalf("thread still present: found=%v err=%v", found, err)
	}
	messages, err := memory.RecentMessages(context.Background(), "thread-1", 10)
	if err != nil {
		t.Fatal(err)
	}
	if len(messages) != 0 {
...
func TestThreadDeleteRefusesAThreadWithAWorkspaceAttached(t *testing.T) {
	...
	if err := memory.AttachWorkspace(context.Background(), "thread-1", "web", "repo", "/tmp/checkout", time.Now()); err != nil {
		t.Fatal(err)
	}
	handler := newThreadDeleteHandler(memory)
...
func TestThreadDeleteReturns404ForAnUnknownThread(t *testing.T) {
	...
	request := withPathValue(httptest.NewRequest(http.MethodDelete, "/api/chat/threads/missing", nil), "id", "missing")
	response := httptest.NewRecorder()
	handler(response, request)
