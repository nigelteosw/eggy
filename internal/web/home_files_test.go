...
func TestLogsAreReadableButNotWritable(t *testing.T) {
	files, layout := newHomeFiles(t)
	if err := os.WriteFile(filepath.Join(layout.Logs(), home.GatewayLogName), []byte("hello"), 0o600); err != nil {
		t.Fatal(err)
	}
	content, access, err := files.Read("logs/" + home.GatewayLogName)
	if err != nil || content != "hello" || access != AccessRead {
		t.Fatalf("content=%q access=%q err=%v", content, access, err)
	}
	if err := files.Write("logs/"+home.GatewayLogName, "tampered"); !errors.Is(err, ErrFileReadOnly) {
