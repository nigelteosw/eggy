...
// older one. What changed is where the compare-and-set happens -- a single
...
func loadState(ctx context.Context, source rows) (ports.State, error) {
	state := initialState()
	var (
		approvalMode string
		autoMode     int
	...
	err := source.QueryRowContext(ctx, `SELECT version, approval_mode, approval_auto_mode, agent, repositories FROM machine_state WHERE id = 1`).
	...
	if err := scanRows(ctx, source, `SELECT id, record FROM approvals`, func(scan func(...any) error) error {
	...
	if err := scanRows(ctx, source, `SELECT id, seen_at FROM processed_events`, func(scan func(...any) error) error {
	...
	if err := scanRows(ctx, source, `SELECT sent_at FROM proactive_messages ORDER BY id`, func(scan func(...any) error) error {
		var sentAt string
...
func saveState(ctx context.Context, tx *sql.Tx, state ports.State) error {
	...
	if _, err := tx.ExecContext(ctx, `
		INSERT INTO machine_state (id, version, approval_mode, approval_auto_mode, agent, repositories)
		VALUES (1, ?, ?, ?, ?, ?)
		...
		state.Version, string(state.ApprovalMode), autoMode, string(agent), string(repositories)); err != nil {
		return fmt.Errorf("write machine state: %w", err)
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM approvals`); err != nil {
		return err
	}
	for id, approval := range state.Approvals {
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO approvals (id, record) VALUES (?, ?)`, id, string(record)); err != nil {
			return fmt.Errorf("write approval %s: %w", id, err)
		}
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM processed_events`); err != nil {
		return err
	}
	for id, handled := range state.ProcessedEvents {
		if _, err := tx.ExecContext(ctx, `INSERT INTO processed_events (id, seen_at) VALUES (?, ?)`, id, formatTime(handled)); err != nil {
			return fmt.Errorf("write processed event %s: %w", id, err)
		}
	}
	if _, err := tx.ExecContext(ctx, `DELETE FROM proactive_messages`); err != nil {
		return err
	}
	for _, sent := range state.ProactiveMessages {
		if _, err := tx.ExecContext(ctx, `INSERT INTO proactive_messages (sent_at) VALUES (?)`, formatTime(sent)); err != nil {
...
func scanRows(ctx context.Context, source rows, query string, visit func(scan func(...any) error) error) error {
	result, err := source.QueryContext(ctx, query)
	if err != nil {
		return err
	}
	defer result.Close()
